<template>
  <div>
    <h1 class="primary-font dark-purple">
      <span v-if="recipe.originalRecipe">Add a new recipe</span>
    ...
    <div class="form-group">
      <input
        class="form-control"
        :class="{ 'is-invalid': errors.name && !recipe.name }"
        placeholder="Name"
        v-model="recipe.name"
      />
      <div
        v-if="errors.name && !recipe.name"
        class="dark-purple form-text is-invalid"
      >{{errors.name}}</div>
    </div>
    ...
    <div class="form-group">
      <input
        class="form-control"
        :class="{ 'is-invalid': errors.cookTime && !recipe.cookTime }"
        placeholder="Cook Time"
        v-model="recipe.cookTime"
      />
      <div
        v-if="errors.cookTime && !recipe.cookTime"
        class="dark-purple form-text is-invalid"
      >{{errors.cookTime}}</div>
    </div>
    ...
    <div class="form-group">
      <input
        class="form-control"
        :class="{ 'is-invalid': errors.prepTime && !recipe.prepTime }"
        placeholder="Prep Time"
        v-model="recipe.prepTime"
      />
      <div
        v-if="errors.prepTime && !recipe.prepTime"
        class="dark-purple form-text is-invalid"
      >{{errors.prepTime}}</div>
    </div>
    ...
    <div v-if="recipe.originalRecipe">
      ...
      <div id="preview">
        ...
        <img
          v-if="recipe.imageUrl && !tempImageURLList"
          :src="recipe.imageUrl"
    ...
    <div v-else>
      ...
      <h2
        class="primary-font dark-purple"
      >Want different photos? Delete this recipe and start from scratch!</h2>
    </div>
    ...
    <div class="form-row" v-for="(ingredient, index) in recipe.ingredients">
      ...
      <div class="col-md-10 mb-3">
        ...
        <div
          v-if="errors.ingredients && !recipe.ingredients[index].food"
          class="dark-purple form-text is-invalid"
    ...
    <div class="form-row" v-for="(instruction, index) in recipe.instructions">
      <div class="col-md-1 mb-3">
        ...
        <input
          ...
          class="form-control"
          tabindex="-1"
          style="background-color:transparent; border: 0; font-size: 1em;"
          readonly
        />
      </div>
      <div class="col-md-10 mb-3">
        ...
        <div
          v-if="errors.instructions && !recipe.instructions[index].do"
          class="dark-purple form-text is-invalid"
        >{{errors.instructions}}</div>
      </div>
      <div class="col-md-1 mb-3 center-text">
        ...
        <font-awesome-icon
          ...
          size="lg"
          v-if="index != 0"
          @click="shiftStepUp('instructions', index)"
        ></font-awesome-icon>
        ...
        <font-awesome-icon
          ...
          @click="shiftStepDown('instructions', index)"
        ></font-awesome-icon>
        ...
        <font-awesome-icon
          ...
          size="lg"
          v-if="index != 0"
          @click="removeStep('instructions', index)"
        ></font-awesome-icon>
      </div>
    </div>
  ...
  computed: {
    errorsPresent() {
      ...
      for (var key in this.errors) {
        ...
        if (this.errors[key])
          return "Check above for errors - You may have missed something important!";
      }
  ...
  methods: {
    addRecipe(e) {
      ...
      if (this.recipe.originalRecipe) {
        axios
          ...
          .catch(function(error) {
            ...
            for (var message in validationMessages) {
              currentObj.errors[message] =
                "Oops! " + validationMessages[message];
            }
          });
      } else {
        ...
        axios
          .post("/editrecipe/" + this.recipe.id, formData, {
            headers: { "content-type": "multipart/form-data" }
          })
          .then(function(response) {
            console.log(response.data);
    ...
    shiftStepUp(objectKey, index) {
      ...
      this.recipe[objectKey].splice(
        index - 1,
        2,
        this.recipe[objectKey][index],
        this.recipe[objectKey][index - 1]
      );
    ...
    removeStepWithBackspace(objectKey, index) {
      if (objectKey == "ingredients") {
        if (
          this.recipe[objectKey][index].amount == "" &&
          this.recipe[objectKey][index].food == ""
        ) {
